...
static void edid_recheck_state(struct tegra_hpd_data *data)
{
	...
	if (recheck_edid(data, &match)) {
		 ...
		 */
		if (match) {
			pr_info("hpd: No EDID change, but resetting hpd state machine anyway to force new link training.\n");
		} else {
...
static void handle_hpd_evt(struct tegra_hpd_data *data, int cur_hpd)
{
	...
	if (data->req_suspend) {
		...
		/*
		 * Did HPD drop while we were in DONE_ENABLED ? If so,
		 * Reset the system.
		 */
		tgt_state = STATE_HPD_RESET;
		...
		if (!tegra_dc_ext_is_userspace_active()) {
			 ...
			 * reset regardless.
			 */
			pr_info("hpd: resetting on bouncing hpd\n");
			tgt_state = STATE_HPD_RESET;
			timeout = HPD_STABILIZE_MS;
		}
