...
/*
 * Link training failure at the lowest bit rate normally will move to the
 * DONE_FAIL state. Add support to 'retry' the link training again, starting
 * at the RESET_STATE.
 */
#define DP_LT_MAX_RETRIES 5
					...
					bool is_clk_recovery)
{
	if (is_clk_recovery)
		...
		lt_data->aux_rd_interval ? msleep(lt_data->aux_rd_interval * 4) :
...
static void get_lt_new_config(struct tegra_dp_lt_data *lt_data)
{
	...
	bool pc_supported = lt_data->pc_supported;
	u32 sec = 0;
	...
	for (cnt = 0; cnt < 4; cnt++) {
		tegra_dc_dp_dpcd_read(lt_data->dp,
			(NV_DPCD_SYMBOL_ERROR_COUNT_LANE0 + cnt), &data_ptr);
		sec = sec | ((u32)data_ptr << (8 * cnt));
	}

	for (cnt = 0; cnt < n_lanes; cnt++)
		pr_info("dp lt: new config: lane %d: "
			"vs level: %d, pe level: %d, pc2 level: %d, sec: %#X\n",
			cnt, vs[cnt], pe[cnt], pc_supported ? pc[cnt] : 0, sec);
}
...
static void lt_data_sw_reset(struct tegra_dp_lt_data *lt_data)
{
	...
	/* post cursor controls were deprecated in DP1.3 */
	lt_data->pc_supported = (dp->revision <= 0x12);
...
static void lt_passed(struct tegra_dp_lt_data *lt_data)
{
	...
	/* reset retry count for future disconnects */
	lt_data->lt_retry = 0;
	mutex_unlock(&lt_data->lock);
}
...
fail:
	lt_failed(lt_data);
	if (lt_data->lt_retry++ > DP_LT_MAX_RETRIES) {
		pr_info("dp lt: bit rate already lowest!\n");
		set_lt_state(lt_data, STATE_DONE_FAIL, -1);
		/* reset retry count for future disconnects */
		lt_data->lt_retry = 0;
	} else {
		pr_info("dp lt: link training fail, going to retry...\n");
		dp->link_cfg.is_valid = true;
		tegra_dp_update_link_config(dp);
		set_lt_state(lt_data, STATE_RESET, 0);
	}
