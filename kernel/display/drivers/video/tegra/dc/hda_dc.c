...
static struct mutex hda_lock[MAX_SOR_COUNT] = {
		__MUTEX_INITIALIZER(hda_lock[SOR0]),
		__MUTEX_INITIALIZER(hda_lock[SOR1]),
};
static int audio_freq_array[MAX_SOR_COUNT];
					...
					int sor_num)
{
	...
	if (valid_freq) {
		...
		audio_freq_array[sor_num] = audio_freq;
...
void tegra_hda_set_data(struct tegra_dc *dc, void *data, int sink)
{
	...
	tegra_hdmi_setup_audio_freq_source(audio_freq_array[sor_num], HDA, sor_num);
}
