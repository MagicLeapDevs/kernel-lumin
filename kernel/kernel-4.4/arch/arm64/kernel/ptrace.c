...
asmlinkage int syscall_trace_enter(struct pt_regs *regs, unsigned int sbox_action)
{
	if (sbox_filter_syscall(regs->syscallno, sbox_action) != 0)
		return -1;

	/* Do the secure computing check; failures should be fast. */
