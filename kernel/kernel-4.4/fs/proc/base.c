...
static int proc_pid_debuggable(struct seq_file *m, struct pid_namespace *ns,
			struct pid *pid, struct task_struct *task)
{
	seq_printf(m, "%d", task->is_debuggable);
	return 0;
}
				 ...
				 int hide_pid_min)
{
	...
	/* init process need to be able to check why a zombie
	 * was killed (read killed_by_lowmem) in order to
	 * maintain the correct crash count
	 */
	if (task_tgid_nr(current) == 1 && capable(CAP_DAC_OVERRIDE))
		return true;
...
static int proc_killed_by_lowmem(struct seq_file *m, struct pid_namespace *ns,
				 struct pid *pid, struct task_struct *task)
{
	char killed_by_lowmem = '0';

	/* flag is set in lowmemorykiller.c, lowmem_scan() */
	if (test_tsk_thread_flag(task, TIF_ML_LOWMEMDIE))
		killed_by_lowmem = '1';

	seq_putc(m, killed_by_lowmem);
	seq_putc(m, '\n');
	return 0;
}
...
static const struct pid_entry tgid_base_stuff[] = {
	...
	ONE("debuggable", S_IRUGO, proc_pid_debuggable),
	...
	ONE("killed_by_lowmem", S_IRUGO, proc_killed_by_lowmem),
#ifdef CONFIG_AUDITSYSCALL
