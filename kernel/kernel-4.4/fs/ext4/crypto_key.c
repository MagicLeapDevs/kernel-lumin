...
char global_fbe_key[EXT4_AES_256_XTS_KEY_SIZE];
char global_fbe_desc[EXT4_KEY_DESC_PREFIX_SIZE +
				 (EXT4_KEY_DESCRIPTOR_SIZE * 2) + 1];
bool global_fbe_added;
...
int ext4_get_encryption_info(struct inode *inode)
{
	...
	if (IS_ERR(keyring_key)) {
		if (global_fbe_added &&
		   (!strncmp(global_fbe_desc, full_key_descriptor,
				   EXT4_KEY_DESC_PREFIX_SIZE + EXT4_KEY_DESCRIPTOR_SIZE * 2))) {
			res = ext4_derive_key(&ctx,
				  global_fbe_key, raw_key);
			keyring_key = NULL;
			if (res)
				goto out;
			goto got_key;
		...
		pr_warn(
			"Error: Failed to find key with descriptor: %s (inode num: %u)\n",
			full_key_descriptor,  (unsigned) inode->i_ino);
