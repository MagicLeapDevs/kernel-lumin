...
static unsigned long pax_disable_debug_flags(unsigned long pax_flags)
{
	/* Allow RWX allocations */
	pax_flags &= ~MF_PAX_MPROTECT;

	return pax_flags;
}

static long pax_parse_pax_flags(const struct elfhdr * const elf_ex, const struct elf_phdr * const elf_phdata, struct file * const file, bool elf_debuggable)
{
	unsigned long pax_flags, ei_pax_flags,  pt_pax_flags, xattr_pax_flags;

#ifdef CONFIG_MLSEC_SANDBOX
	/* Only parse additional PaX flags if sandbox allows */
	bool allow_additions = current->sandbox_profile->enable_jit;
#else
	...
	/* Disable certain PaX flags on debuggable executables */
	if (elf_debuggable)
		pax_flags = pax_disable_debug_flags(pax_flags);
...
static int load_elf_binary(struct linux_binprm *bprm)
{
	...
	/* Apply sandbox on the ELF */
	retval = sbox_apply(bprm);
	if (retval)
		goto out;
...
elf_debuggable = is_debuggable(bprm->file);
#ifdef CONFIG_MLSEC_HARDEN_PTRACE
	/* If ptrace was requested, verify that target is debuggable */
	/* (This handles the case of fork() + PTRACE_TRACEME + exec() */
	if (current->ptrace)
		if (!elf_debuggable) {
			retval = -EPERM;
			goto out_free_dentry;
...
#if defined(CONFIG_PAX_NOEXEC) || defined(CONFIG_PAX_ASLR)
	if (0 > pax_parse_pax_flags(&loc->elf_ex, elf_phdata, bprm->file, elf_debuggable)) {
		retval = -EINVAL;
		goto out_free_dentry;
	}
#endif
...
#ifdef CONFIG_PAX_MPROTECT
/* PaX: We'll remove VM_MAYWRITE for good on RELRO segments.
 *
