...
static void max77620_pm_power_off(void *drv_data)
{
	...
	/* set the low battery threshold for the next power up sequence */
	if (max77620_poweroff->lbdac_thres >= 0) {
		ret = regmap_update_bits(max77620_poweroff->rmap,
			MAX77620_REG_CNFGGLBL1,
			MAX77620_CNFGGLBL1_LBDAC_MASK,
			(max77620_poweroff->lbdac_thres <<
			 MAX77620_CNFGGLBL1_LBDAC_SHIFT));
		if (ret < 0)
			dev_err(max77620_poweroff->dev,
				"CNFGGLBL1 LBDAC val update failed: %d\n",
				ret);
	}
...
static int max77620_poweroff_probe(struct platform_device *pdev)
{
	...
	if (np) {
		...
		of_property_read_u32(np, "maxim,low-battery-threshold",
						&lowbatt_thres);
		if (system_pc) {
			use_power_off = true;
...
gpio_done:
	...
	max77620_poweroff->lbdac_thres = lowbatt_thres;
