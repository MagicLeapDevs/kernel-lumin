		...
		struct device_node **chg_np, struct device_node **vbus_np)
{
	...
	if (batt_reg_node) {
		...
		count = of_property_count_u32_elems(batt_reg_node, "ti,soc-range");
		...
		count = of_property_count_u32_elems(batt_reg_node, "ti,temp-range");
...
vbus_node:
	...
	if (vbus_reg_node) {
		...
		pdata->vbus_pdata->ridata = of_get_regulator_init_data(
					&client->dev, vbus_reg_node, &bq2419x_vbus_reg_desc);
