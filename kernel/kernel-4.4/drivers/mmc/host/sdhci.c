...
void sdhci_set_uhs_signaling(struct sdhci_host *host, unsigned timing)
{
	...
	if ((timing == MMC_TIMING_MMC_HS200) ||
		...
		(host->quirks2 & SDHCI_QUIRK2_SEL_SDR104_UHS_MODE_IN_SDR50))
		ctrl_2 |= SDHCI_CTRL_UHS_SDR104;
	else if (timing == MMC_TIMING_UHS_SDR50)
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask, u32 *mask)
{
	...
	if (intmask & SDHCI_INT_TIMEOUT)
		...
		host->cmd->error = -EILSEQ;
		pr_err("\n%s  %s  %d   SDHCI_INT_END_BIT ",
				mmc_hostname(host->mmc), __func__, __LINE__);
		sdhci_dumpregs(host);
	} else if (intmask & SDHCI_INT_INDEX) {
		host->cmd->error = -EILSEQ;
		pr_err("\n%s  %s  %d   SDHCI_INT_INDEX ",
				mmc_hostname(host->mmc), __func__, __LINE__);
		sdhci_dumpregs(host);
	}
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	ret = request_threaded_irq(host->irq, sdhci_irq, sdhci_thread_irq,
				   IRQF_SHARED,	dev_name(mmc->parent), host);
	if (ret) {
