			...
			uint32_t regaddr, uint8_t data)
{
	...
	if (JEDEC_MFR(flash->flash_info->jedec_id) == JEDEC_MICRON)
		return -ENXIO;
...
static int qspi_quad_flag_set(struct qspi *flash, uint8_t is_set)
{
	...
	if ((flash->is_quad_set && is_set) ||
		...
		(JEDEC_MFR(flash->flash_info->jedec_id) == JEDEC_MICRON)) {
		return status;
	}
				...
				uint8_t erase_opcode, u32 size)
{
	...
	t[0].len = (COMMAND_WIDTH + flash->addr_width);
	t[0].tx_buf = cmd_addr_buf;
		...
		size_t *retlen, u_char *buf)
{
	...
	if (cdata) {
		if (len > cdata->x1_len_limit) {
			if (cdata->x4_is_ddr) {
				...
				copy_cmd_default(&flash->cmd_table,
					&cmd_info_table[QUAD_IO_READ_4]);
			} else {
				copy_cmd_default(&flash->cmd_table,
					&cmd_info_table[QUAD_IO_READ_3]);
			}
		} else {
			copy_cmd_default(&flash->cmd_table,
	...
	pr_debug("\topcode: %#x, cmd ddr: %d, cmd bus_width: %d\n",
		...
		size_t *retlen, const u_char *buf)
{
	...
	enum qspi_operation_mode op_mode;
	struct tegra_qspi_device_controller_data *cdata =
					flash->spi->controller_data;
	...
	if (flash->addr_width == 4)
		op_mode = PAGE_PROGRAM_4;
	else
		op_mode = PAGE_PROGRAM_3;
...
static const struct spi_device_id *jedec_probe(struct spi_device *spi)
{
	...
	pr_err("unrecognized JEDEC id %#06x / ext %#04x\n", jedec, ext_jedec);
...
static int qspi_probe(struct spi_device *spi)
{
	...
	if ((info->jedec_id & 0xFF) > 0x18) {
		flash->addr_width = 4;
	} else {
		flash->addr_width = 3;
		flash->erase_opcode = OPCODE_SE_3;
	}
	...
	if (JEDEC_MFR(flash->flash_info->jedec_id) != JEDEC_SPANSION)
		goto done;
