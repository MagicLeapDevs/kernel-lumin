...
#define pte_offset_phys_idx(dir,idx)      (pmd_page_paddr(*(dir)) + idx * sizeof(pte_t))
#define pte_offset_kernel_idx(dir,idx)    ((pte_t *)__va(pte_offset_phys((dir), (idx))))
				   ...
				   unsigned long pfn, int prot, int stage)
{
	...
	start = pte_offset_kernel(pmd, addr);
	pte = start;
	...
	do {
		...
		if (arm_smmu_pte_is_contiguous_range(addr, end, __pfn_to_phys(pfn))) {
			...
			cont_start = pte_offset_kernel_idx(pmd, idx);
