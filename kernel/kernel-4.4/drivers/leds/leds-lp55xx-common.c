...
static int lp55xx_check_enable(struct lp55xx_chip *chip)
{
	struct lp55xx_device_config *cfg = chip->cfg;
	u8 addr = cfg->enable.addr;
	u8 val = 0;
...
int lp55xx_init_device(struct lp55xx_chip *chip)
{
	...
	/* skips reset if chip is already initialized */
	ret = lp55xx_check_enable(chip);
	if (!ret) {
		dev_info(dev, "LP55xx device already configured.\n");
		return ret;
	}
