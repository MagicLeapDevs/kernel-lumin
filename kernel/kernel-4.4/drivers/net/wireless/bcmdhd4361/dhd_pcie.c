 ...
 *
 * $Id: dhd_pcie.c 734141 2017-12-01 08:16:43Z $
 */
	...
	dhd_bus_t *bus, bool *is_murata_fem)
{
	...
	const naming_info_t *info = NULL;
	const char *cid_info = NULL;
...
dhdpcie_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	if (imgbuf == NULL) {
		DHD_ERROR(("%s: dhd_os_open_image(%s) failed\n", __FUNCTION__, pfw_path));
		goto err;
	}
...
void tegra_pcie_prepare_l2_exit(dhd_pub_t *dhdp)
{
	...
	tegra_pcie_pm_control(TEGRA_PCIE_RESUME_PRE, bus->dev);
}
#endif
