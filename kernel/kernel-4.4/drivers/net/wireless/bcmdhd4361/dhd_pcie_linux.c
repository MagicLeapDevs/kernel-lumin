 ...
 *
 * $Id: dhd_pcie_linux.c 702624 2017-06-02 08:45:41Z $
 */
...
#endif /* CONFIG_ARCH_MSM */
#ifdef CUSTOMER_TK1
#if defined(CONFIG_ARCH_TEGRA) || defined(CONFIG_PCI_TEGRA)
	bcmerror = tegra_pcie_pm_resume();
#endif /* CONFIG_ARCH_TEGRA || CONFIG_PCI_TEGRA */
#endif /* CUSTOMER_TK1 */
	if (bcmerror < 0) {
...
void dhdpcie_port_toggle(dhd_pub_t *pub, bool on)
{
	...
	if (!bus) {
		DHD_ERROR(("%s: bus is NULL, port toggle failed\n",
			   __FUNCTION__));
		return;
	}
	if (on)
		tegra_pcie_port_enable_per_pdev(bus->dev);
	else
		tegra_pcie_port_disable_per_pdev(bus->dev);
}
#endif
