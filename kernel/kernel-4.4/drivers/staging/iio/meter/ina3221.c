...
#define INA3221_WEN_LATCH_BIT		BIT(11)
#define INA3221_CEN_LATCH_BIT		BIT(10)
		...
		struct device_attribute *attr, char *buf)
{
	...
	case VBUS_VOLTAGE_CURRENT:
		...
		return ret;
	case ALARM_VALUE:
		if (!gpio_is_valid(chip->pdata->crit_warn_gpio)) {
			dev_err(dev, "Alarm IRQ not supported\n");
			return -ENODEV;
		}

		/* Alarm value is active low */
		ret = gpio_get_value(chip->pdata->crit_warn_gpio);
		return snprintf(buf, PAGE_SIZE, "%s\n",
				ret ? "inactive" : "active");
	default:
		break;
...
static IIO_DEVICE_ATTR(alarm_value, S_IRUGO,
		ina3221_show_channel, NULL,
		PACK_MODE_CHAN(ALARM_VALUE, 0));
...
static struct attribute *ina3221_attributes[] = {
	...
	&iio_dev_attr_alarm_value.dev_attr.attr,
	NULL,
};
	...
	struct i2c_client *client)
{
	...
	pdata->crit_warn_gpio = of_get_named_gpio(np, "crit-warn-gpio", 0);
	if (gpio_is_valid(pdata->crit_warn_gpio)) {
		ret = devm_gpio_request_one(dev, pdata->crit_warn_gpio,
					    GPIOF_IN |
					    GPIOF_ACTIVE_LOW,
					    "INA_CRIT_WARN_LIMIT");
		if (ret) {
			dev_err(dev, "Failed to request gpio: %d with %d\n",
				pdata->crit_warn_gpio, ret);
			return ERR_PTR(ret);
		}
	}
...
static irqreturn_t crit_warn_irq_handler(int irq, void *client_id)
{
	const struct i2c_client *i2cclient = (struct i2c_client *)client_id;
	const struct iio_dev *indio_dev = i2c_get_clientdata(i2cclient);
	struct kobject *kobj = (struct kobject *)&indio_dev->dev.kobj;

	sysfs_notify(kobj, NULL, "alarm_value");

	return IRQ_HANDLED;
}

static int crit_warn_irq_init(struct i2c_client *client, int gpio)
{
	int ret;
	int mask;
	int flags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT;

	/* If there is no interrupt support, don't register for alarms */
	if (!gpio_is_valid(gpio)) {
		dev_warn(&client->dev, "interrupt support disabled\n");
		return 0;
	}

	ret = devm_request_threaded_irq(&client->dev, gpio_to_irq(gpio),
					NULL, &crit_warn_irq_handler,
					flags, "crit_warn_irq", client);
	if (ret) {
		dev_err(&client->dev, "IRQ handler failure: %d\n", ret);
		return ret;
	}

	ret = i2c_smbus_read_word_data(client, INA3221_MASK_ENABLE);
	if (ret < 0)
		return ret;

	/* Configure the warning/critical alarms as 'passthrough' and skip
	 * latching the sources which become cleared on next read.
	 */
	mask = be16_to_cpu(ret);
	mask &= ~(INA3221_WEN_LATCH_BIT | INA3221_CEN_LATCH_BIT);
	mask = cpu_to_be16(mask);

	ret = i2c_smbus_write_word_data(client, INA3221_MASK_ENABLE, mask);
	if (ret)
		return ret;
			...
			const struct i2c_device_id *id)
{
	...
	ret = crit_warn_irq_init(client, pdata->crit_warn_gpio);
	if (ret)
		goto exit_pd;
