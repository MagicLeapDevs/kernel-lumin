...
static int l2r_fpga_adjust_freq(struct ml_fpga_io *io, const u32 __user *val)
{
	struct fpga_data *data = io_2_pdata(io);
	u32 freq_adj[FPGA_FREQ_ADJ_LEN];
	int rc;

	rc = copy_from_user(freq_adj, val, sizeof(freq_adj));
	if (rc) {
		dev_err(data->dev, "%s: copy_from_user failed, %d\n",
			__func__, rc);
		return rc;
	}

	mutex_lock(&data->l2r_lock);
	rc = l2r_write(data, L2R_CMD_CORDIC_MEM, 0x06, freq_adj,
		       FPGA_FREQ_ADJ_LEN);
	mutex_unlock(&data->l2r_lock);
	if (rc)
