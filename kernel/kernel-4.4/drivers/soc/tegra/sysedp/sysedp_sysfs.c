...
/*
 * Creating a sysedp consumer from user space.
 * Write a string with the consumer name and power state
 * information to this sysfs interface.
 * "echo consumer_name state0, state1, …, stateN >
 * /sys/power/sysedp/consumer_register"
 */
...
/*
 * Removing a sysedp consumer which is created from user space.
 * "echo consumer_name > /sys/power/sysedp/consumer_unregister"
 */
static ssize_t consumer_unregister_store(const char *s, size_t count)
{
	...
	strlcpy(name, s, n);
	consumer = sysedp_unregister_consumer_by_name(strim(name));
	if (!consumer)
		return -EINVAL;
