...
{
	fixed20_12 win_width, win_height;
	__u32 h_downscale_ratio, v_downscale_ratio;
	/*
	 * Verify downscaling factor for T186
	 * Maximum downscaling in any direction is 4x.
	 * Up-scaling is not limited, but only verified up to 16x.
	 * For performance reasons, vertical down-scaling should not exceed 2x.
	 * For performance reasons, total down-scaling (H x V) should not
	 * exceed 4x.
	 */
	if (win->out_w && win->out_h) {
		win_width.full = win->w;
		win_height.full = win->h;
		if (win->flags & TEGRA_DC_EXT_FLIP_FLAG_SCAN_COLUMN) {
			/* Rotated 90 degrees or 270 degrees */
			v_downscale_ratio =
				dfixed_trunc(win_width) / win->out_h;
			h_downscale_ratio =
				dfixed_trunc(win_height) / win->out_w;
		} else {
			/* Either no rotation or 180 degrees rotation */
			v_downscale_ratio =
				dfixed_trunc(win_height) / win->out_h;
			h_downscale_ratio =
				dfixed_trunc(win_width) / win->out_w;
		}

		if (v_downscale_ratio > 2) {
			dev_err(&dc->ndev->dev,
				"Vertical downscale ratio greater than 2x\n");
			return -EINVAL;
		}

		if ((h_downscale_ratio * v_downscale_ratio) > 4) {
			dev_err(&dc->ndev->dev,
			"Total (H x V) downscale ratio greater than 4x\n");
			return -EINVAL;
		}
	}
...
static inline u32 tegra_nvdisp_win_swap_uv(struct tegra_dc_win *win)
{
	...
	case TEGRA_WIN_FMT_T_Y12___U12V12_N444:
		if (!(win->fmt & TEGRA_DC_EXT_FMT_BYTEORDER_SWAP4HW))
			swap_uv = 1;
		break;
	}
				      ...
				      bool wait_for_vblank)
{
	...
	if (yuv && (NVDISP_ODD_VAL(dfixed_trunc(win->h)) ||
		...
		dev_err(&dc->ndev->dev,
		"Win %d HEIGHT(%u),WIDTH(%u) must be EVEN for YUV fmts(%u)\n",
		win->idx, dfixed_trunc(win->h), dfixed_trunc(win->w), win->fmt);
		err = -EINVAL;
		goto attr_fail;
	}
	...
	if (yuv && (NVDISP_ODD_VAL(dfixed_trunc(h_offset))
		...
		dev_err(&dc->ndev->dev,
		"Win %d X(%u),Y(%u) offsets must be EVEN for YUV fmts(%u)\n",
		win->idx, dfixed_trunc(h_offset), dfixed_trunc(v_offset),
		win->fmt);
		err = -EINVAL;
		goto attr_fail;
	}
